...
class DiscountTagWidget extends StatelessWidget {
  ...
  const DiscountTagWidget({super.key,
    ...
    this.inLeft = true, this.isBogoTag = false,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return (isBogoTag || discount! > 0 || freeDelivery!) ? Positioned(
      ...
      child: CustomPaint(
        ...
        child: Container(
          ...
          child: Text(
           isBogoTag ? 'bogo'.tr : discount! > 0 ? '${(isRightSide || discountType == 'percent') ? '' : currencySymbol}$discount${discountType == 'percent' ? '%'
